...
- `.last-updated` — the "Last updated [date]" line in the sidebar, present on every page except the landing page (`index.html`). Update it by find-and-replace across all 12 sidebar pages whenever something substantive changes.
...
## Contact
Overview's closing paragraph includes a mailto link (`gavin_m_hamilton@yahoo.com`). Update the address directly in `overview.html` if it ever needs to change.
